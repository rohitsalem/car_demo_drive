...
namespace gazebo
{
  class PriusHybridPluginPrivate
  {
    ...
    /// \brief PID control for cruize
	public: common::PID cruizepPID;
    ...
    : dataPtr(new PriusHybridPluginPrivate)
{
  ...
  //ros::NodeHandle nh;
  this->dataPtr->controlSub = this->dataPtr->nh.subscribe("prius", 10, &PriusHybridPlugin::OnPriusCommand, this);
  this->dataPtr->steeranglePub= this->dataPtr->nh.advertise<prius_msgs::SteeringAngle>("prius/steering_angle", 1 );
...
void PriusHybridPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  // For the cruize control
  this->dataPtr->cruizepPID.Init(100,10,10,0,0,1,-1);
...
void PriusHybridPlugin::Update()
{
  ...
  // this->dataPtr->gasPedalPercent = 0.15;
  //ROS publisher for steering angle
      this->dataPtr->steering_angle.data=this->dataPtr->handWheelAngle;
      this->dataPtr->steering_angle.header.stamp=ros::Time::now();
      this->dataPtr->steeranglePub.publish(this->dataPtr->steering_angle);
